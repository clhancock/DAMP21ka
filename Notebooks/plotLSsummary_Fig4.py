...
for age in ['21k','PI']:
    ...
    #
    gcm[age]['PETpr'] = da_utils_ls.calculatePET(
        tas       =  gcm[age]['tas'],
        press     =  gcm[age]['ps'],
        relhum    =  gcm[age]['hurs'],
        netrad    =  gcm[age]['radnet'],
        wind      =  gcm[age]['LM19_U2']) #not needed for priestley_taylor
...
age='combined'
gcm[age]['LakeStatusPET']   = da_utils_ls.calcLakeStatus(runoff=gcm[age]['mrros'],precip=gcm[age]['pr'],levap=gcm[age]['PETpr'],Qmethod='runoff')
gcm[age]['LakeStatusLEVAP'] = da_utils_ls.calcLakeStatus(runoff=gcm[age]['mrros'],precip=gcm[age]['pr'],levap=gcm[age]['LM19_lakeEvap'],Qmethod='runoff')
#%% Plot fig S4 
#PS4a Scatter Plots of lake status based on evap
fs=8
...
ax.set_ylabel('runoff / '+r'$(E_{{potential}} - P)$'         ,fontsize=fs)
ax.tick_params(labelsize=fs)    
#Save
plt.tight_layout()
plt.savefig(wd1+'Figures/Fig4. CCSM4 LGM-PI/FigS4a_LakeLevelScatter_'+age+'_runoff.png',dpi=600)
plt.show()

# Plot fig S4a Scatter Plots of lake evap
...
ax.set_xlabel('lake evaporation (mm/day)',fontsize=fs)
ax.set_ylabel('potential evaporation (mm/day)',fontsize=fs)
...
#Save
plt.tight_layout()
plt.savefig(wd1+'Figures/Fig4. CCSM4 LGM-PI/FigS4a_EvaporationScatter_'+age+'.png',dpi=600)
plt.show()
#%%Map Plots
...
for i,var in enumerate(['LakeStatusLEVAP','LakeStatusPET']):
    name =[['(a) ',r'$\frac{Runoff}{E_{{lake}} - P}$'+' (original)','LowryMorrill'],# '\n(Lowry & Morrill, 2019)',
            ['(b) ',r'$\frac{Runoff}{E_{{potential}} - P}$'+'(DAMP21ka)','DAMP21ka'], #'\n(Hancock et al., (this Study))',
            ][i]
    ...
    #Save
    plt.tight_layout()
    plt.savefig(wd1+'Figures/Fig4. CCSM4 LGM-PI/Fig4_LakeLevel_Difference_LGM-PI_'+name[2]+'.png',dpi=600)
    plt.show()
...
for method in ['r2']:
    ...
    #Save
    plt.tight_layout()
    plt.savefig(wd1+'Figures/Fig4. CCSM4 LGM-PI/Fig4_LakeLevel_r2map_'+age+'.png',dpi=600)
    plt.show()
    #plt.savefig(wd+'
    #%%